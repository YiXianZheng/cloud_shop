...
public interface ResponseCode {
    ...
    public enum Base implements ResponseCode {
         ...
         * 基础返回码--库存不足
         */
        OUT_OF_STOCK(20, "商品库存不足", "Insufficient inventory of goods"),
        /**
         * 基础返回码--库存不足
         */
        GET_LOCK_ERR(21, "获取锁失败", "Get lock error");
    ...
    public static ResponseCode getByCode(String code){
        ...
        for (ResponseCode type : Base.values()){
            if(code.equals(type.getCode()+"")){
                return type;
            }
        }
        for (ResponseCode type : UNINTENDED_RESULT.values()){
            if(code.equals(type.getCode()+"")){
                return type;
            }
        }
        for (ResponseCode type : Parameter.values()){
            if(code.equals(type.getCode()+"")){
                return type;
            }
        }
        for (ResponseCode type : LoginRegister.values()){
            if(code.equals(type.getCode()+"")){
                return type;
            }
        }
        for (ResponseCode type : bankcard.values()){
            if(code.equals(type.getCode()+"")){
                return type;
            }
        }
        for (ResponseCode type : SecurityCode.values()){
            if(code.equals(type.getCode()+"")){
                return type;
            }
        }
